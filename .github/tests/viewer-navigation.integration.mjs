...
assert.match(printStyleSource, /\.print-chapter-door \+ \.print-heading-stack\s*\{[\s\S]*break-before:\s*page/);
...
assert.match(appSource, /function preparePrintHierarchy\(\)[\s\S]*className = "print-heading-stack"[\s\S]*classList\.add\("print-chapter-door"\)/);
