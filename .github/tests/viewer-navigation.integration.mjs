...
assert.match(indexSource, /value="standard" selected>標準（Chapter扉）/);
...
assert.match(appSource, /const PRINT_SETTINGS_KEY = "vocab-viewer-print-settings-v1"/);
assert.match(appSource, /function readStoredPrintSettings\(\)[\s\S]*localStorage\.getItem\(PRINT_SETTINGS_KEY\)/);
assert.match(appSource, /function initialPrintSetting\(paramName\)[\s\S]*PAGE_PARAMS\.has\(paramName\)[\s\S]*storedPrintSettings\[paramName\]/);
assert.match(appSource, /function savePrintSettings\(\)[\s\S]*localStorage\.setItem\(PRINT_SETTINGS_KEY, JSON\.stringify\(settings\)\)/);
assert.match(appSource, /\.forEach\(\(control\) => control\.addEventListener\("change", handlePrintSettingChange\)\)/);
...
assert.match(printStyleSource, /body:not\(\[data-print-pagination="section"\]\) \.entry\s*\{[\s\S]*padding:\s*0\.78rem 0 0\.78rem 3\.3rem/);
assert.match(printStyleSource, /body:not\(\[data-print-pagination="section"\]\) \.entry-no\s*\{[\s\S]*width:\s*2\.75rem/);
...
assert.match(appSource, /function preparePrintIllustrationWrapping\(\)[\s\S]*printPagination === "section"[\s\S]*entryCard\.prepend\(illustration\)[\s\S]*has-print-anchored-illustration/);
