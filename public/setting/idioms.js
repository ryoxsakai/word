...
function replaceSavedIdiom(saved){const previous=current?.key;data.entries=data.entries.filter(entry=>entry.key!==saved.key&&entry.key!==previous);data.entries.push(saved);const oldKey=current?.sectionKey;if(oldKey&&oldKey!==saved.sectionKey)sectionEntries.set(String(oldKey),(sectionEntries.get(String(oldKey))||[]).filter(entry=>entry.key!==saved.key));const key=String(saved.sectionKey),rows=(sectionEntries.get(key)||[]).filter(entry=>entry.key!==saved.key);sectionEntries.set(key,orderIdiomLabels(resolveIdiomReferences([...rows,saved],words),flatSections().find(section=>section.key===key)?.labels));refreshReferences();refreshSectionOptions();renderTable();}
