...
function entryNumbers(){const numbers=new Map();let no=0;for(const section of flatSections())for(const entry of data.entries.filter(item=>item.sectionKey===section.key&&!item.hidden))numbers.set(entry.key,String(++no));return numbers;}
...
function hydrateSection(sectionKey,entries){const resolved=resolveIdiomReferences(entries,words);sectionEntries.set(String(sectionKey),resolved);const byId=new Map(resolved.map(entry=>[entry.key,entry]));data.entries=data.entries.map(entry=>byId.has(entry.key)?{...entry,...byId.get(entry.key)}:entry);}
