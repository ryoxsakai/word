...
const el = {
  ...
  printPagination: document.getElementById("printPagination"),
...
function normalizedPrintPagination(rawValue) {
...
function applyPrintSettings() {
  ...
  document.documentElement.dataset.printPagination = pagination;
  document.body.dataset.printPagination = pagination;
...
async function ensureIdioms() {
  ...
  const listId = state.currentListId;
  const generation = listLoadGeneration;
  const skeletonRow = `<div class="idiom-loading-row">
    <span class="skeleton-headword skeleton-line"></span>
    <span class="skeleton-meaning skeleton-line"></span>
  </div>`;
...
function renderLabeledIdiomEntries(section) {
  ...
  return section.items.map(item => {
    ...
    const heading = key && key !== previous ? `<div class="label-divider" role="heading" aria-level="5">${hierarchyIcon("label")}<span class="label-title">${escapeHtml(label.name)}</span></div>` : '';
    previous = key;
    return heading + renderIdiomEntry(item, VIEWER_API_BASE, {resolve:resolveRef, renderNotes:state.renderNotesMarkup});
  }).join('');
}
...
function renderIdioms({ deferNavigation = false } = {}) {
  ...
  el.idiomList.innerHTML = groups.length ? groups.map(chapter => chapter.sections.map((section, index) => {
    ...
    return `<section class="section-group chapter-tone-${chapter.tone}${index === 0 ? " has-chapter-divider" : ""}${startsGroup ? " has-group-divider" : ""}" aria-labelledby="${sectionId}">${heading}${groupHeading}
      ...
      ${idiomSectionIsLoaded(section.key) ? `<div class="idiom-entries" data-idiom-section-entries="${escapeHtml(String(section.key))}">${renderLabeledIdiomEntries(section)}</div>` : idiomSectionSkeleton(section)}
    </section>`;
...
function openDedicatedPrintView() {
  ...
  url.searchParams.set("pagination", el.printPagination.value);
...
function preparePrintHierarchy() {
  ...
  const firstChapterDoor = el.wordList.querySelector(":scope > .print-chapter-door");
  if (firstChapterDoor) el.wordList.before(firstChapterDoor);
}
