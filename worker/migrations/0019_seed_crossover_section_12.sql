...
VALUES
  (1, 'change', 1, '変化・変換',
   ...
   'transform' || char(59) || ' convert', NULL, 1, 0, 1),
  (2, 'transform', 1, '変化・変換',
   '形・性質を大きく変える語。',
   'change' || char(59) || ' convert', NULL, 0, 0, 0),
  (3, 'convert', 1, '変化・変換',
   '用途・形式・信念などを別のものに転換する。',
   'change' || char(59) || ' transform', NULL, 0, 0, 0),
  (4, 'translate', 1, '変化・変換',
  ...
  (7, 'develop', 2, '形成・発達',
   ...
   'grow' || char(59) || ' evolve', NULL, 1, 0, 1),
  (8, 'evolve', 2, '形成・発達',
  ...
  (10, 'divide', 3, '加工・分割',
   ...
   'split' || char(59) || ' separate', NULL, 1, 0, 1),
  (11, 'split', 3, '加工・分割',
   'divideより「割れる・分裂する」感触が強い。',
   'divide' || char(59) || ' separate', 'split – split – split', 1, 1, 0),
  (12, 'break', 3, '加工・分割',
  ...
  (14, 'group', 4, '分類・整理',
   ...
   'classify' || char(59) || ' sort', NULL, 1, 0, 0),
  (15, 'sort', 4, '分類・整理',
   'sort A into Bは種類別に分ける。sort out OはOを整理・解決する。',
   'classify' || char(59) || ' group', NULL, 1, 0, 0),
  (16, 'organize', 4, '分類・整理',
  ...
  (19, 'incorporate', 5, '挿入・統合',
   ...
   'integrate' || char(59) || ' include', NULL, 0, 0, 0),
  (20, 'integrate', 5, '挿入・統合',
   '複数部分を機能する全体へ統合する語。integrate with Bも用いる。',
   'incorporate' || char(59) || ' combine', NULL, 1, 0, 0);
